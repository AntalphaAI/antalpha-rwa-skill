...
_config_path_env = os.environ.get("ANTALPHA_RWA_CONFIG")
CONFIG_FILE = (
    Path(_config_path_env)
    if _config_path_env
    else SCRIPT_DIR.parent / "references" / "mcp.json"
)
...
def _retry_on_session_expired(func):
    """Decorator to retry MCP calls when session expires (401/403)."""
...
def mcp_initialize() -> str:
    ...
    payload = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "antalpha-rwa-client", "version": "1.0"},
        },
        "id": 1,
    }
...
def get_orders_by_address(wallet_address: str) -> List[Dict[str, Any]]:
    ...
    """
    result = call_mcp_tool("query-orders-by-address", {"address": wallet_address})
...
def generate_payment_link(
    ...
    # OKX Web3 deep link
    okx_link = (
        f"okx://wallet/dapp/details?dappUrl={urllib.parse.quote(eip681_link, safe='')}"
    )

    # Trust Wallet deep link
    trust_link = f"https://link.trustwallet.com/open_url?coin_id=60&url={urllib.parse.quote(eip681_link, safe='')}"

    # TokenPocket deep link
    tp_params = json.dumps({"url": eip681_link, "chain": "ETH"})
    tokenpocket_link = f"tpdapp://open?params={urllib.parse.quote(tp_params, safe='')}"

    return {
        "eip681": eip681_link,
        "metamask": metamask_link,
        "okx": okx_link,
        "trustwallet": trust_link,
        "tokenpocket": tokenpocket_link,
...
def calculate_returns(
    amount: float, annual_yield: float, term_days: int
) -> Dict[str, float]:
    """
...
def cmd_orders(args):
    ...
    try:
        ...
        for i, order in enumerate(orders, 1):
            ...
            print(
                f"   Product: {order.get('product_name', order.get('product', 'N/A'))}"
            )
            print(
                f"   Amount: {order.get('amount', 'N/A')} {order.get('token', 'USDT')}"
            )
            print(f"   Status: {order.get('status', 'N/A')}")
            print(
                f"   Subscribed: {order.get('subscribed_at', order.get('created_at', 'N/A'))}"
            )
...
def cmd_products(args):
    ...
    try:
        ...
        for i, p in enumerate(products, 1):
            ...
            receiving_address = (
                p.get("receive_address_base")
                or p.get("receive_address_eth")
                or p.get("receivingAddress", "N/A")
            )
...
def cmd_subscribe(args):
    ...
    try:
        ...
        # Support both old and new API field names
        min_sub = float(
            product.get("min_subscription", product.get("minSubscriptionUsdt", 10))
        )
        ...
        receiving_address = (
            product.get("receive_address_base")
            if chain == "base"
            else product.get("receive_address_eth")
        )
        ...
        # Generate payment link (includes all wallet deeplinks)
        ...
        # Generate QR code (single EIP-681 link for all wallets)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        qr_path = str(SCRIPT_DIR.parent / f"payment_qr_{timestamp}.png")
        generate_qr_code(payment["eip681"], qr_path)
        ...
        print(f"💎 OKX Web3:    {payment['okx']}")
        print(f"🛡️ Trust Wallet: {payment['trustwallet']}")
        print(f"📱 TokenPocket: {payment['tokenpocket']}")
        print(f"\n" + "-" * 60)
        print("QR Code")
        print("-" * 60)
        if qr_path:
            print(f"\nGenerated: {qr_path}")
            print(f"Scan with any supported wallet to pay")
        else:
        ...
        print(f"\n" + "-" * 60)
        print(
            f"⚠️  Send exactly {args.amount} {token} on {chain_name} to complete subscription."
        )
        print("-" * 60 + "\n")
    ...
    try:
        ...
        qr_path_metamask = str(
            SCRIPT_DIR.parent / f"payment_qr_metamask_{timestamp}.png"
        )
        ...
        if args.json:
            output = {
                ...
                "payment": payment,
                "qr_codes": {"metamask": qr_path_metamask, "simple": qr_path_simple},
            }
            print(json.dumps(output, indent=2))
            return 0
        ...
        if qr_path_simple:
            ...
            print(
                f"    ⚠️  For OKX, TokenPocket, etc. Enter amount manually: {args.amount} {token}"
            )
        else:
            print(f"    Failed to generate QR code")
        print(f"\n[4] Manual Transfer:")
...
def main():
    ...
    orders_parser = subparsers.add_parser(
        "orders", help="Query orders by wallet address"
    )
    orders_parser.add_argument(
        "--address", type=str, required=True, help="Wallet address to query orders"
    )
    ...
    subscribe_parser.add_argument(
        "--amount", type=float, required=True, help="Investment amount in USDT"
    )
    subscribe_parser.add_argument(
        "--chain", type=str, default=None, help="Chain name (base or ethereum)"
    )
    subscribe_parser.add_argument(
        "--token", type=str, default=None, help="Token symbol (USDT or USDC)"
    )
    subscribe_parser.add_argument(
        "--qr", type=str, help="Generate QR code and save to path"
    )
